...
# ---------- Chatbot demo ---------- #
class WebChatbotOnboardWorld(OnboardWorld):
    """
    ...
    def generate_world(opt, agents):
        return WebChatbotOnboardWorld(opt=opt, agent=agents[0])

    def parley(self):
        self.episodeDone = True


class WebChatbotTaskWorld(World):
    """
    ...
    def generate_world(opt, agents):
        ...
        return WebChatbotTaskWorld(
            opt,
            agents[0],
            create_agent_from_shared(
                opt['shared_bot_params'][WebChatbotTaskWorld.MODEL_KEY]
            ),
        )
...
class WebChatOverworld(World):
    ...
    @staticmethod
    def generate_world(opt, agents):
        return WebChatOverworld(opt, agents[0])
